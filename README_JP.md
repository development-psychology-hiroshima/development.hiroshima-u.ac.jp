...
[English](https://github.com/development-psychology-hiroshima/development.hiroshima-u.ac.jp/blob/master/README.md)

このマニュアルは，広島大学発達心理学研究室の[ホームページ](https://development.hiroshima-u.ac.jp)を正常に動かせるために作成したもの。ご不明な点などございましたら，ぜひご気楽に[作者](mailto:chenzhaolu_deer@outlook.com)までお問い合わせください。

## 構成ファイルの編集

大方のページ内容は， `/(public)/configs` ディレクトリの `main.yml` に見られます。（先生2人の研究活動と，年間スケジュールの三つのページ除外）

構成ファイルを編集する時に，**日本語の全角スペース"　"の代わりに，英語の半角スペース" "を使用してください。**不具合が発生する原因となります。
...
members:
  -
    name: Aさん
    # 下の欄は全角スペースが付けっているので，構成ファイルを読み込める時にエラーが発生します。
　　position: 博士課程前期1年
```

また，**２−４半角スペースの字下げを前文と同じように維持してください。**不具合が発生する原因となります。
...
members:
  -
    name: Aさん
    # 下の欄の字下げが前文と違うので，構成ファイルが読み込める可能性はありますが，内容を正しくディスプレイできない場合となります。
position: 博士課程前期1年
```

構成ファイルは正しく読み込めるかどうかを知りたいときに，以下の手順に従って調べてみてください。

## サイトの構成ファイルをチェックする手順

1. モバイル以外の設備（ノートパソコンや，iPadなどのタブレット）を通じて，発達研HPをアクセスしてください。
2. メニュー欄の先頭の英文字は大文字か小文字かを確認してください。
3. 大文字の場合，構成ファイルは正しく認識していると推定できます。小文字の場合は，パソコンの `F12` キーを押し，Console 欄に赤いエラー情報を見つけて，エラー情報を参照して構成ファイルを再編集してください。

<details>
<summary>正確の構成ファイルの一例は下記通り</summary>
